...
class Pedro_Player:
  ...
  def discard(self,trump_suit):
    self.cards=[card for card in self.cards if card.check_trump(trump_suit,'pedro')]
  ...
  def hand_as_image(self,frame):
    ...
    for count,card in enumerate(self.cards):
      shift=count*dx
      hand_frame.create_image(shift+x_center,y_center,image=card.image)
    return hand_frame
  ...
  def bid_frame(self,frame):
    ...
    if self.dealer:
      ...
      b.pack(side='left')
      for bid in bids:
        b=tk.Button(bid_frame,text=bid,font=button_font,command=quit)
        self.buttons.append(b)
        b.pack(side='left')
        b['state']=tk.DISABLED
    else:
      text='Pass'
      b=tk.Button(bid_frame,text=text,font=button_font,command=lambda: game.get_bid(
        game.order_loc,game.current_bid,game.winning_bid_loc))
      self.buttons.append(b)
      b.pack(side='left')
      b['state']=tk.DISABLED
      for bid in bids:
        b=tk.Button(bid_frame,text=bid,font=button_font,command=lambda bid=bid: game.get_bid(
          game.order_loc+1,bid,game.order_loc))
        self.buttons.append(b)
        b.pack(side='left')
        b['state']=tk.DISABLED
