...
class Card:
  ...
  def __init__(self, rank, suit,scale=0.6):
    ...
    self.photo_image=ImageTk.PhotoImage(self.image.resize((int(self.SCALE*self.WIDTH),
      int(self.SCALE*self.HEIGHT)),Image.ANTIALIAS))
  ...
  def resize(self,scale):
    self.scale=scale
    self.photo_image=ImageTk.PhotoImage(self.image.resize((int(scale*self.WIDTH),
      int(scale*self.HEIGHT)),Image.ANTIALIAS))
...
class Pedro_Player:
  ...
  def get_label(self,frame,fontsize,fontcolor):
    ...
    label=tk.Label(frame,text=self.name,bg='forest green',fg=fontcolor,font=font_style)
    return label
  ...
  def hand_as_image(self,frame,scale=0.5):
    ...
    for count,card in enumerate(self.hand.cards):
      shift=count*dx
      hand_frame.create_image(shift+x_center,y_center,image=card.photo_image)
    ...
    for count,card in enumerate(self.hand.cards):
      ...
      if count == len(self.hand.cards) - 1:
        buttons[count]=tk.Button(hand_frame,height=height,
            width=width,image=card.photo_image,command=quit)
      else:
        buttons[count]=tk.Button(hand_frame,height=height,
            width=dx,image=card.photo_image,command=quit)
...
class Trick:
  ...
  def old_trick(self,frame,scale=0.25):
    ...
    for count,card in enumerate(self.cards):
      hand_frame.create_image(width*count+x_center,y_center,image=card.photo_image)
...
frame=tk.Frame(root,height = screen_height, width = screen_width,bg='forest green',borderwidth=10,relief='ridge')
